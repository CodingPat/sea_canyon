...
class Joueur(sprite.Sprite):
    ...
    def __init__(self,x,y,image,vies=3,fichierson=None):
...
class Musique():
    """
        Classe pour charger et jouer de la musique
        Le fichier doit être dans le répertoire /music
    """
    def __init__(self,fichier):
        
        #vérifier si le mixer a pu être initialisé avant de charger la musique
        if pygame.mixer:
            #chemin complet pour le fichier
            fichier = os.path.join(rep_principal, 'music', fichier)
            pygame.mixer.music.load(fichier)
        
    def jouer(self, repetition=-1,volume=1):
        """
            Joueur musique
            sans fin : repetition=-1
        """
    ...
    def arreter(self):
        """
            Arreter la musique après un délai en millisecondes
            Pendant ce délai, le programme est bloqué
        """
        pygame.mixer.music.stop()

class Son():
    """
        Classe pour charger et jouer un son
        Le fichier doit être dans le répertoire /music
    """
    ...
    def __init__(self,fichier):
        ...
        if pygame.mixer:
            #chemin complet pour le fichier
            fichier = os.path.join(rep_principal, 'music', fichier)
            self.sound=pygame.mixer.Sound(fichier)
            self.channel=pygame.mixer.find_channel()
        else:
            self.sound=None
            self.channel=None
             
    def jouer(self, repetition=0,volume=1):
        """
            Joueur musique
            sans fin : repetition=-1
        """
        if self.channel:
            ...
            self.channel.play(self.sound,repetition)
...
class MoteurJeu():
    ...
    def __init__(self,largeurecran,hauteurecran,titre):
        ...
        self.typeecranactuel=""# demarrage,findepartie,collision,pause    
    ...
    def findepartie(self):
        ...
        if self.typeecranactuel!="findepartie":
            self.typeecranactuel="findepartie"
            ...
            musique=Musique(musiquefindepartie)
            musique.jouer(-1,0.01)
        ...
        while True:
            ...
            if self.estecrancollision:
                ...
                if not(self.joueur.vies>0):
                    ...
                    musique=Musique(musiquejeuactif)
                    musique.jouer(-1,1)
                
                                
            if self.estecrandemarrage:
                if self.typeecranactuel !="demarrage":
                    self.typeecranactuel="demarrage"
                    self.reset_jeu()
                    font1=pygame.font.SysFont("arialblack",48)
                    font2=pygame.font.SysFont("arialblack",24)
                    color=(0,0,255)
                    ...
                    musique=Musique(musiqueecrandemarrage)
                    musique.jouer(-1,0.01)
...
if __name__=="__main__":
    largeurecran=800
    hauteurecran=600
    rep_principal=os.path.split(os.path.abspath(__file__))[0]    
    #placer les fichiers dans le sous-répertoire music
    musiqueecrandemarrage="276557__tyops__arcade-intro.ogg"
    musiquejeuactif="255597__akemov__underwater-ambience.ogg"
    musiquefindepartie="276557__tyops__arcade-intro.ogg"
    sonexplosion="159402__noirenex__overheadexplosion.ogg"    
    pygame.init()
    if pygame.mixer and not pygame.mixer.get_init():
        print ('ERREUR d\'initialisation du mixer. PAS de son')
        pygame.mixer = None

    moteurJeu=MoteurJeu(largeurecran,hauteurecran,"Sea canyon !")
    moteurJeu.joueur=Joueur(largeurecran/2,hauteurecran/2,"squidred.png",3,sonexplosion)
